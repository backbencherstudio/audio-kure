  ...
  return (
    <div className='border-t mt-5 border-[#2f2861]'>
      <div className='max-w-7xl mx-4 md:mx-auto my-8'>
        <div className='text-3xl font-semibold my-8'>Your cure session for Day {selectedDay}</div>
          ...
          <div className='flex flex-col gap-4'>
            {currentSession && (
              <div className='relative rounded-3xl overflow-hidden shadow-lg'>
                <img src={currentSession.image} alt="" className='opacity-70 w-full' />
                <div className='absolute inset-0 flex flex-col justify-end items-center bg-gradient-to-t from-black to-transparent p-4'>
                  <div className='text-white text-3xl font-bold mb-2'>Session {selectedDay}</div>
                  <div className='text-white text-xl font-semibold mb-2'>{currentSession.title}</div>
                  <div className='text-white mb-4 text-center font-semibold'>
                    Description for {currentSession.title} goes here.
                  </div>
                  {currentAudio ? (
                    <CustomAudioPlayer audioSrc={currentAudio} onAudioEnd={() => setCurrentAudio(null)} />
                  ) : (
                    <button
